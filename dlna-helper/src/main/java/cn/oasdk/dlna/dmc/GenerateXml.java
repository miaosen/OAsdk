...
public class GenerateXml {
	...
	public String generate(Item Item,Container container) throws Exception {
		return generate(Item,container, false);
	}

	public String generate(Item Item,Container container, boolean nestedItems)
			throws Exception {
		return documentToString(buildDOM(Item, container,nestedItems), true);
	}
	...
	protected Document buildDOM(Item Item,Container container, boolean nestedItems)
			throws Exception {
		...
		generateRoot(Item, container,d, nestedItems);

		return d;
	}

	protected void generateRoot(Item Item,Container container, Document descriptor,
			boolean nestedItems) {
		...
		//
		// for (DescMeta descMeta : Item.getContainer().getDescMetadata()) {
		// if (descMeta == null)
		// continue;
	...
	protected void populateDescMetadata(Element descElement, DescMeta descMeta) {
		if (descMeta.getMetadata() instanceof Document) {
			...
			log.warning("Unknown desc metadata Item, please override populateDescMetadata(): "
