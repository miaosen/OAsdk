...
public class ImagePickerAdapter extends RecyclerView.Adapter<ImagePickerAdapter.BaseHolder> {
    ...
    private void bindMedia(MediaHolder mediaHolder, MediaFile mediaFile) {
        ...
        Uri imageUri = mediaFile.getUri();
        if (!TextUtils.isEmpty(imagePath)) {
            //选择状态（仅是UI表现，真正数据交给SelectionManager管理）
            if (SelectionManager.getInstance().isImageSelect(imageUri.toString())) {
            ...
            try {
                ConfigManager.getInstance().getImageLoader().loadImage(mediaHolder.mImageView, imagePath, mediaFile.getUri());
            } catch (Exception e) {
                e.printStackTrace();
            }
