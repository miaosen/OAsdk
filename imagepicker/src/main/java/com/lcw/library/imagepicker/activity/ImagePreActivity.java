...
public class ImagePreActivity extends BaseActivity {
            ...
            public void onClick(View v) {
                ...
                if (ConfigManager.getInstance().isSingleType()) {
                    ...
                    if (!selectPathList.isEmpty()) {
                        //判断选中集合中第一项是否为视频
                        if (!SelectionManager.isCanAddSelectionPaths(mMediaFileList.get(mViewPager.getCurrentItem()).getUri().toString(), selectPathList.get(0))) {
                            //类型不同
                ...
                boolean addSuccess = SelectionManager.getInstance().addImageToSelectList(mMediaFileList.get(mViewPager.getCurrentItem()).getUri().toString());
                if (addSuccess) {
                    updateSelectButton(mMediaFileList.get(mViewPager.getCurrentItem()).getUri().toString());
                    updateCommitButton();
                } else {
    ...
    protected void getData() {
        //String imageList = getIntent().getStringExtra(IMAGE_LIST);
        //if(imageList!=null){
        //    mMediaFileList=new LinkedList<>();
        //    try {
        //        JSONArray jsonArray=new JSONArray(imageList);
        //        for (int i = 0; i < jsonArray.length(); i++) {
        //            JSONObject jsonObject = jsonArray.getJSONObject(i);
        //            MediaFile mediaFile=new MediaFile();
        //            mediaFile.setPath( jsonObject.getString("path"));
        ...
        //}
       /* Log.i("logtag","============getData==========="+mMediaFileList);
        for (int i = 0; i < mMediaFileList.size(); i++) {
            MediaFile mediaFile = mMediaFileList.get(i);
            Log.i("logtag","============getData==========="+mediaFile.getPath());
            Log.i("logtag","============getData==========="+mediaFile.getFolderName());
            Log.i("logtag","============getData==========="+mediaFile.getMime());
            Log.i("logtag","============getData==========="+mediaFile.getDuration());
            Log.i("logtag","============getData==========="+mediaFile.getDateToken());
            Log.i("logtag","============getData==========="+mediaFile.getFolderId());
        }*/
